...
class User:
    ...
    """
    def __init__(self, email, username, tz, password, salt = bcrypt.gensalt(), last_code = -1, valid_until = str(datetime.now()), verified = False):
        self.email = email
...
def check_code(email, code):
    ...
    if(str_to_date(user.valid_until) < datetime.now()):
        return "time"
    elif (int(user.last_code) != int(code)):
        return "code"
    else:
        return "ok"
