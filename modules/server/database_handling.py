...
database = f"{os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))}\\DataBase\\users.db"
table_name = "users"
...
def check_table():
    ...
    conn = sqlite3.connect(database)
    cursor = conn.cursor()
    cursor.execute(f"CREATE TABLE IF NOT EXISTS {table_name} (email TEXT PRIMARY KEY, username TEXT UNIQUE, tz TEXT, password TEXT, salt TEXT, last_code INTEGER, valid_until TEXT, verified BOOL)")
    conn.commit()
    conn.close()
...
def row_to_dict(row):
    ...
    """
    user_dict = {"email": row[0], "username": row[1], "tz": row[2], "password": row[3],"salt": row[4],"last_code": row[5],"valid_until": row[6],"verified": bool(row[7]),}
    return user_dict
...
def get_user(cred):
    ...
    conn = sqlite3.connect(database)
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS users (email TEXT PRIMARY KEY, username TEXT UNIQUE, tz TEXT, password TEXT, salt TEXT, last_code INTEGER, valid_until TEXT, verified BOOL)")
